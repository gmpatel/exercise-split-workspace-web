                ...
                case "openclaim":
                    {
                        ...
                        return Helper.ProcessClaimNumber(messageCode, workspaceId, claimNumber);
                    }
                case "openwork":
                    {
                        var workReference = (postData.ContainsKey("workReference") ? postData["workReference"].ToString() : string.Empty);

                        return Helper.ProcessWorkReference(messageCode, workspaceId, workReference);
                    }
                case "opensinglework":
                    {
                        var workId = (postData.ContainsKey("workId") ? postData["workId"].ToString() : string.Empty);

                        return Helper.ProcessWorkId(messageCode, workspaceId, workId);
                    }
                ...
                case "openpolicy":
                    {
                        ...
                        return Helper.ProcessPolicyNumber(messageCode, workspaceId, policyNumber);
                    }
                case "unlockwork":
                    {
                        var workId = (postData.ContainsKey("workId") ? postData["workId"].ToString() : string.Empty);

                        return Helper.ProcessUnlockWork(messageCode, workspaceId, workId);
                    }
                default:
                    {
